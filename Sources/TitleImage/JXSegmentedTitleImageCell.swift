...
open class JXSegmentedTitleImageCell: JXSegmentedTitleCell {
    ...
    open override func layoutSubviews() {
        ...
        if (myItemModel.selectedImageInfo ?? "").isEmpty == false {
            ...
            switch myItemModel.titleImageType {
                ...
                case .rightImage:
                    let contentWidth = imageSize.width + myItemModel.titleImageSpacing + titleLabel.bounds.size.width
                    if (myItemModel.normalImageInfo ?? "").isEmpty {
                        titleLabel.center = CGPoint(x: titleLabel.bounds.size.width * 0.5, y: contentView.bounds.size.height/2)
                    } else {
